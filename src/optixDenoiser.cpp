...
void OptiXDenoiser::init(Data &data) {
  ...
  SUTIL_ASSERT_MSG(!data.normal || data.albedo,
  ...
  //
  {
    ...
    options.inputKind            = data.normal   ? OPTIX_DENOISER_INPUT_RGB_ALBEDO_NORMAL
    ...
    OPTIX_CHECK(optixDenoiserSetModel(m_denoiser, OPTIX_DENOISER_MODEL_KIND_HDR,
                                      nullptr, // data
                                      0        // size
                                      ));
  }
  ...
  //
  {
    ...
    OPTIX_CHECK(
        optixDenoiserComputeMemoryResources(m_denoiser, data.width, data.height, &denoiser_sizes));
    ...
    CUDA_CHECK(cudaMemcpy(reinterpret_cast<void *>(m_inputs[0].data), data.color, frame_byte_size,
    ...
    if (data.albedo) {
      ...
      CUDA_CHECK(cudaMemcpy(reinterpret_cast<void *>(m_inputs[1].data), data.albedo,
                            frame_byte_size, cudaMemcpyHostToDevice));
    ...
    if (data.normal) {
      ...
      CUDA_CHECK(cudaMemcpy(reinterpret_cast<void *>(m_inputs[2].data), data.normal,
                            frame_byte_size, cudaMemcpyHostToDevice));
  ...
  //
  {
    OPTIX_CHECK(optixDenoiserSetup(m_denoiser,
                                   0, // CUDA stream
                                   data.width, data.height, m_state, m_state_size, m_scratch,
                                   m_scratch_size));
...
void OptiXDenoiser::exec() {
  OPTIX_CHECK(optixDenoiserComputeIntensity(m_denoiser,
                                            0, // CUDA stream
                                            m_inputs, m_intensity, m_scratch, m_scratch_size));

  OPTIX_CHECK(optixDenoiserInvoke(m_denoiser,
                                  0, // CUDA stream
                                  &m_params, m_state, m_state_size, m_inputs,
                                  m_inputs[2].data   ? 3
                                  : m_inputs[1].data ? 2
                                                     : 1, // num input channels
                                  0,                      // input offset X
                                  0,                      // input offset y
                                  &m_output, m_scratch, m_scratch_size));
...
void OptiXDenoiser::finish() {
  ...
  CUDA_CHECK(cudaMemcpy(m_host_output, reinterpret_cast<void *>(m_output.data), frame_byte_size,
